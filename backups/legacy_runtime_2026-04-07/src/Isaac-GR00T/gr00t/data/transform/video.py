...
try:
    ...
    TORCHVISION_TRANSFORMS_AVAILABLE = True
except Exception:
    T = None
...
class VideoTransform(ModalityTransform):
    ...
    _INTERPOLATION_MAP: ClassVar[dict[str, dict[str, Any]]] = PrivateAttr(
        {
            "nearest": {
                ...
                "torchvision": T.InterpolationMode.NEAREST if T is not None else None,
            },
            "linear": {
                "albumentations": cv2.INTER_LINEAR,
                "torchvision": T.InterpolationMode.BILINEAR if T is not None else None,
            },
            "cubic": {
                "albumentations": cv2.INTER_CUBIC,
                "torchvision": T.InterpolationMode.BICUBIC if T is not None else None,
            },
            "area": {
            ...
            "lanczos4": {
                ...
                "torchvision": T.InterpolationMode.LANCZOS if T is not None else None,  # Torchvision does not specify filter size, might be different from 4x4
            },
            "linear_exact": {
            ...
            "nearest_exact": {
                ...
                "torchvision": T.InterpolationMode.NEAREST_EXACT if T is not None else None,
            },
            "max": {
    ...
    def _require_torchvision(self):
        if not TORCHVISION_TRANSFORMS_AVAILABLE:
            raise ImportError("torchvision transforms are not available.")
